  ...
  return (
    <>
      ...
      {[...Array(5).keys()].map((_, index) => (
        <div className="flex items-center justify-between bg-[#1f1f1f] text-white p-3 rounded-lg w-full my-4">
          {/* Rank */}
          <div className="text-xl font-bold w-12">#{index + 1}</div>
          ...
          {/* Release Date */}
          <div className="text-sm text-gray-400 w-28 text-center">
            Nov 4, 2023
          </div>

          {/* Album Name */}
          <div className="text-sm text-gray-200 flex-1 text-center truncate">
            Hard to Imagine the Neighbourhood Ever Changing
          </div>

          {/* Like + Duration */}
          <div className="flex items-center gap-2 text-sm w-20 justify-end">
            <Heart size={16} className="text-[#62d962]" />
            <span>3:26</span>
          </div>
        </div>
      ))}
